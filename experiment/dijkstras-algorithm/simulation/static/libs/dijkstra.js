...
function Dijkstra(canvas, myfont, isdigraph, nodes, edges, isPrim) {
  ...
  Node.prototype.paint = function (context, node) {
    // override Node.paint
    if (node.stat == STAT.fix) {
      // fixed
      context.strokeStyle = "rgb(40, 142, 200)";
      context.fillStyle = "rgb(40, 142, 200)";
    } else if (node.stat == STAT.sel) {
      // selected
      context.strokeStyle = "rgb(40, 142, 200)";
      context.fillStyle = "gray";
    } else if (node.stat == STAT.adj) {
      // adjacent
  ...
  Node.prototype.setlabelPos = function () {
    ...
    // local function
    function weight(v0, x1, y1) {
      // weight of direction (x1, y1)
      var w = 0;
      var z;
      var x0 = Nodes[v0].xy;
  ...
  //
  function mycomp(i, j) {
    // compare labels of node(i) and node(j)
    // used in the heap
    var d1 = Nodes[i].dist;
    var d2 = Nodes[j].dist;
    return d1 - d2 > 0;
  ...
  function step2() {
    ...
    // local functions
    function nextDPof(j) {
      return Edges[j].deltaP;
    }
    function nextDMof(j) {
      return Edges[j].deltaM;
    }
    function initVof(j) {
      return Edges[j].initv;
    }
    function termVof(j) {
      return Edges[j].termv;
    }
